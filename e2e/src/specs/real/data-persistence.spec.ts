...
test.describe('Data Persistence', { tag: [Tag.DASHBOARD, Tag.SERVICES, Tag.CLIENTS, Tag.APPOINTMENTS, Tag.CRITICAL, Tag.HAPPY_PATH] }, () => {
  ...
  test('services page loads with real catalog data', async ({ authenticatedPage }) => {
    ...
    // Real catalog should have some services from the seeded tenant
    await authenticatedPage.waitForTimeout(2000);
    const cards = authenticatedPage.locator('[role="button"]').filter({ hasText: /\$/ });
  ...
  test('finances page loads with real data', async ({ authenticatedPage }) => {
    ...
    await expect(authenticatedPage.getByText(/analítica|inteligencia/i).first()).toBeVisible({ timeout: 5000 });
  });
