...
export class MockProvider implements ApiProvider, DomainDataProvider {
  ...
  private async fulfillWithOverrides(
    ...
    return route.fulfill({
      ...
      body: JSON.stringify(overridden.body),
    });
  }

  async setup(page: Page, user?: TestUser | null): Promise<void> {
    this.page = page;
    this.database.customers.clear();
    this.database.services.clear();
    this.database.appointments.clear();
    ...
    const fulfill = (route: any, res: ReturnType<typeof json>) =>
      this.fulfillWithOverrides(route, res);
    ...
    const routes = [
      ...
      page.route(API_REQUEST, async (route) => {
        ...
        const entityPaths: Record<string, any> = {
          [API.APPOINTMENTS]: this.database.appointments,
          [API.SERVICES]: this.database.services,
          [API.CUSTOMERS]: this.database.customers,
        };
              ...
              const item =
                apiPath === API.SERVICES
                  ? {
                      id,
                      ...body,
                      price: body.basePrice ?? body.price ?? 0,
                      duration: body.durationMinutes ?? body.duration ?? 0,
                      isActive: true,
                    }
                  : apiPath === API.APPOINTMENTS
                    ? {
                        id,
                        ...body,
                        clientId: body.customerId ?? body.clientId,
                        date: body.startsAt?.split('T')[0],
                        startTime: body.startsAt?.split('T')[1]?.slice(0, 5),
                        endTime: body.endsAt?.split('T')[1]?.slice(0, 5),
                        status: 'pending',
                      }
                    : { id, ...body };
            ...
            if (method === 'PUT') {
              ...
              const patch =
                apiPath === API.SERVICES
                  ? {
                      ...body,
                      price: body.basePrice ?? body.price ?? current?.price,
                      duration: body.durationMinutes ?? body.duration ?? current?.duration,
                    }
                  : body;
        ...
        if (pathname.startsWith('/api/finances'))
          return fulfill(route, json({ revenue: 12500.5, expenses: 3200 }));
        ...
        const isEntityPath = ['appointments', 'services', 'customers', 'artists'].some((p) =>
          pathname.includes(`/api/${p}`)
        );
      ...
      page.route(`**${API.APPOINTMENTS}/*/cancel`, async (route) => {
        ...
        const updated = this.database.appointments.update(id, { status: 'cancelled' });
        return fulfill(route, json(updated ?? { id, status: 'cancelled' }));
      }),
      // Services retire
      page.route(`**${API.SERVICES}/*/retire`, async (route) => {
        const id = new URL(route.request().url()).pathname.split('/').at(-2)!;
        this.database.services.remove(id);
        return fulfill(route, json({}));
      }),
      // Keycloak OIDC mock
      page.route('**/realms/**/.well-known/**', async (route) =>
        fulfill(
          route,
          json({
            issuer: 'http://localhost:8180/realms/emme',
            authorization_endpoint:
              'http://localhost:8180/realms/emme/protocol/openid-connect/auth',
        ...
        (cfg: { auth: boolean; userData: any }) => {
          ...
          sessionStorage.setItem(
            ...
            JSON.stringify({
              ...
              profile: {
                sub: userData.userId || 'default',
                name: userData.name || 'Default',
                email: userData.email || 'default@emme.app',
                preferred_username: userData.userId || 'default',
              },
          ...
          localStorage.setItem(
            'emmenails_cache_isFirstTime',
            JSON.stringify({ data: false, timestamp: Date.now(), version: '1.0.1' })
          );
          localStorage.setItem(
            'emme-ui-state',
  ...
  async seed(data: SeedData): Promise<void> {
    if (data.appointments) this.database.appointments.seed(data.appointments);
    if (data.services) this.database.services.seed(data.services);
    if (data.customers) this.database.customers.seed(data.customers);
  }
  ...
  loadClients = (): Promise<Client[]> => Promise.resolve([...this.database.customers.all()]);
  addClient = (c: CreateClient): Promise<Client> =>
    Promise.resolve(this.database.customers.insert({ id: crypto.randomUUID(), ...c }));
  loadServices = (): Promise<Service[]> => Promise.resolve([...this.database.services.all()]);
  addService = (s: CreateService): Promise<Service> =>
    Promise.resolve(
      this.database.services.insert({ id: crypto.randomUUID(), ...s, isActive: true })
    );
  loadAppointments = (): Promise<Appointment[]> =>
    Promise.resolve([...this.database.appointments.all()]);
  addAppointment = (a: CreateAppointment): Promise<Appointment> =>
    Promise.resolve(this.database.appointments.insert({ id: crypto.randomUUID(), ...a }));
}
