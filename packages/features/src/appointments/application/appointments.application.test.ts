...
describe('appointment application use cases', () => {
  it('lists appointments through the injected repository', async () => {
    ...
    await expect(listAppointments({ appointments: repository })({ date: appointment.date })).resolves.toEqual([
      appointment,
    ]);
  });
  ...
  it('gets an appointment and rejects a missing appointment', async () => {
    ...
    await expect(getAppointment({ appointments: repository })({ appointmentId: appointment.id })).resolves.toEqual(
      appointment,
    );
    await expect(getAppointment({ appointments: repository })({ appointmentId: 'missing' })).rejects.toThrow(
  ...
  it('cancels a cancellable appointment and notifies the injected port', async () => {
    ...
    await expect(
      cancelAppointment({ appointments: repository, notifications })({ appointmentId: appointment.id }),
  ...
  it('delegates availability queries to the injected availability port', async () => {
    ...
    await expect(
      findAvailableSlots({ availability })({ date: appointment.date, serviceId: appointment.serviceId }),
    ).resolves.toEqual([{ startTime: '12:00', endTime: '13:00' }]);
    expect(availability.calls).toEqual([{ date: appointment.date, serviceId: appointment.serviceId }]);
  });
});
