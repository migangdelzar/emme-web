...
- [x] **Step 1:** Write tests for required name/phone, optional fields, duplicate phone detection, retirement, and tenant mismatch.
- [x] **Step 2:** Implement `Customer`, profile value objects, `CustomerRepository`, and typed customer errors.
- [x] **Step 3:** Run domain tests; expected result is PASS.

### Task 2: Application operations

- [x] **Step 1:** Write fake-repository tests for `listCustomers`, `getCustomer`, `createCustomer`, `updateCustomer`, and `retireCustomer`.
- [x] **Step 2:** Implement operations with explicit tenant and permission inputs and injected repository ports.
- [x] **Step 3:** Verify not-found, duplicate, invalid input, unauthorized, and adapter failure cases.

### Task 3: API, validation, and presentation

- [x] **Step 1:** Write payload mapper tests for customer fields, optional values, and backend error envelopes.
- [x] **Step 2:** Implement API operations, schemas, HTTP adapter, `CustomerSummaryCard`, `CustomerProfile`, and reusable empty/error states.
- [x] **Step 3:** Run package tests and `bun run architecture:check`; expected result is PASS.

### Task 4: App migration and commit

- [x] **Step 1:** Update salon routes/components to use the public customer feature API.
- [x] **Step 2:** Create client profile contract exports without adding client pages to the shared module.
- [x] **Step 3:** Run full typecheck/test/build and commit.
...
- [x] Studio customer lifecycle and client profile contracts are reusable and tested.
- [x] Tenant mismatch and sensitive-data redaction are covered.
