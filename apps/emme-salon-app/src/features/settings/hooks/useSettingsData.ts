...
export function useSettingsData() {
  ...
  return {
    ...
    profile: {
      ...
      email: user?.email || '',
      businessName: bizProfile?.businessName || profile?.businessName || tenant?.tenantName || tenant?.tenantSlug || '',
      ...
      notificationsEnabled: bizProfile?.notificationsEnabled ?? profile?.notificationsEnabled ?? false,
    },
    services,
  };
}
