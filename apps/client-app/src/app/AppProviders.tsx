...
export function AppProviders({ children }: { readonly children: ReactNode }) {
  ...
  const http = useMemo(() => createHttpClient({
    baseUrl: resolveClientApiBaseUrl({ VITE_API_BASE_URL: import.meta.env.VITE_API_BASE_URL }, window.location.origin),
    getAccessToken: () => tokens.get().accessToken,
    getTenantSlug: () => storage.get('tenant_slug'),
  }), [storage, tokens]);
